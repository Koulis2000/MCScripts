...
local function DrawHeaders()
	Header_Name = gui.newLabel(mainGui, 4, 2, "Name", 0x212121, 0xd7d787, 30)
	Header_Current = gui.newLabel(mainGui, 35, 2, "Current (Requirement)", 0x212121, 0xd7d787, 18)
	Header_Crafting = gui.newLabel(mainGui, 54, 2, "Crafting", 0x212121, 0xd7d787, 8)
	Header_Message = gui.newLabel(mainGui, 63, 2, "Message", 0x212121, 0xd7d787, 15)
	Header_Line = gui.newHLine(mainGui, 1, 3, 76)
	Header2_Name = gui.newLabel(mainGui, 84, 2, "Name", 0x212121, 0xd7d787, 30)
	Header2_Current = gui.newLabel(mainGui, 115, 2, "Current (Requirement)", 0x212121, 0xd7d787, 18)
	Header2_Crafting = gui.newLabel(mainGui, 134, 2, "Crafting", 0x212121, 0xd7d787, 8)
	Header2_Message = gui.newLabel(mainGui, 143, 2, "Message", 0x212121, 0xd7d787, 15)
...
local function DrawLines()
	...
	for index = 1, 86 do
		if index % 2 == 1 then
			...
			lines[index]["Name"] = gui.newLabel(mainGui, 4, 3 + rowCount, "", 0x212121, 0xd7d787, 30)
			lines[index]["Current"] = gui.newLabel(mainGui, 35, 3 + rowCount, "", 0x212121, 0xd7d787, 18)
			lines[index]["Crafting"] = gui.newLabel(mainGui, 54, 3 + rowCount, "", 0x212121, 0xd7d787, 8)
			lines[index]["Message"] = gui.newLabel(mainGui, 63, 3 + rowCount, "", 0x212121, 0xd7d787, 15)
		else
			lines[index] = {}
			lines[index]["Radio"] = gui.newRadio(mainGui, 81, 3 + rowCount)
			lines[index]["Name"] = gui.newLabel(mainGui, 84, 3 + rowCount, "", 0x212121, 0xd7d787, 30)
			lines[index]["Current"] = gui.newLabel(mainGui, 115, 3 + rowCount, "", 0x212121, 0xd7d787, 18)
			lines[index]["Crafting"] = gui.newLabel(mainGui, 134, 3 + rowCount, "", 0x212121, 0xd7d787, 8)
			lines[index]["Message"] = gui.newLabel(mainGui, 143, 3 + rowCount, "", 0x212121, 0xd7d787, 15)
			rowCount = rowCount + 1
		end
	end
...
local function AddItem_Callback(guiID, buttonID)
	...
	Item_Name_Label = gui.newLabel(addGui, 1, 1, "   Item Name: ", 0x212121, 0xd7d787, 7)
	Item_Name = gui.newText(addGui, 15, 1, 30, "", ItemName_Callback, 30, false)
	Item_NBT_Label = gui.newLabel(addGui, 1, 3, " Item NBT: ", 0x212121, 0xd7d787, 7)
	Item_NBT = gui.newText(addGui, 15, 3, 8, "", ItemNBT_Callback, 8, false)
	Item_NBT_Help = gui.newLabel(addGui, 24, 3, "(Metadata number of item)", 0x212121, 0xd7d787, 7)
	StockRequirement_Label = gui.newLabel(addGui, 1, 5, "    Keep In Stock: ", 0x212121, 0xd7d787, 7)
	StockRequirement = gui.newText(addGui, 15, 5, 8, "", StockRequirement_Callback, 8, false)
	StockRequirement_Help = gui.newLabel(addGui, 24, 5, "(How many items to keep in stock)", 0x212121, 0xd7d787, 7)	
	CraftPerOperation_Label = gui.newLabel(addGui, 1, 7, "Craft Per Operation: ", 0x212121, 0xd7d787, 7)
	CraftPerOperation = gui.newText(addGui, 15, 7, 8, "", CraftPerOperation_Callback, 8, false)
	CraftPerOperation_Help = gui.newLabel(addGui, 24, 7, "(How many items to craft max at once)", 0x212121, 0xd7d787, 7)
...
local function ChangeItem_Callback(guiID, buttonID)
	...
	if radioIndex > 0 then
		...
		Item_Name_Label = gui.newLabel(changeGui, 1, 1, "   Item Name: ", 0x212121, 0xd7d787, 7)
		Item_Name = gui.newText(changeGui, 15, 1, 30, items[changeItemIndex]["rawItemName"], ItemName_Callback, 30, false)
		Item_NBT_Label = gui.newLabel(changeGui, 1, 3, " Item NBT: ", 0x212121, 0xd7d787, 7)
		Item_NBT = gui.newText(changeGui, 15, 3, 8, items[changeItemIndex]["rawItemNBT"], ItemNBT_Callback, 8, false)
		Item_NBT_Help = gui.newLabel(changeGui, 24, 3, "(Metadata number of item)", 0x212121, 0xd7d787, 7)
		StockRequirement_Label = gui.newLabel(changeGui, 1, 5, "    StockRequirement: ", 0x212121, 0xd7d787, 7)
		StockRequirement = gui.newText(changeGui, 15, 5, 8, items[changeItemIndex]["StockRequirement"], StockRequirement_Callback, 8, false)
		StockRequirement_Help = gui.newLabel(changeGui, 24, 5, "(How many items to keep in stock)", 0x212121, 0xd7d787, 7)	
		CraftPerOperation_Label = gui.newLabel(changeGui, 1, 7, "Craft Per Operation: ", 0x212121, 0xd7d787, 7)
		...
		CraftPerOperation_Help = gui.newLabel(changeGui, 24, 7, "(How many items to craft max at once)", 0x212121, 0xd7d787, 7)
...
local function DrawButtons()
	...
	CpuUsageLabel = gui.newLabel(mainGui, 118, 1, "CPU usage: ", 0x212121, 0xd7d787, 13)
	CpuMaxUsageLabel = gui.newLabel(mainGui, 134, 1, "Max CPU usage: ", 0x212121, 0xd7d787, 15)
